...
export default function CardImg({ children, boxes }: Props) {
  ...
  return (
    <picture ref={container} className={s.card} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
      {children}
      <div className={s.boxes}>
        {boxes?.map((box, i) => (
          <div key={i} ref={(r) => void (boxesRef.current[i] = r)} className={box ? s.box : ''}>
            {box}
          </div>
        ))}
      </div>
