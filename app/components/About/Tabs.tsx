...
export default function Tabs({ tabs }: Props) {
  ...
  useGSAP(
    () => {
      ...
      if (isMobile) {
        tl.set(`.${s.line}`, { width: '100%', xPercent: 0 }, 0).to(titles, { xPercent: -100 * currentTab }, 0)
      } else {
  ...
  return (
    <div ref={container} className={s.container}>
      <div className={s.header}>
        {tabs.map((tab, i) => (
          <h3 className={`${s.title} ${i === currentTab ? s.active : ''}`} key={i} onClick={() => setCurrentTab(i)}>
            {tab.title}
          </h3>
        ))}
      ...
      <div className={s.scrollContainer}>
        {tabs.map((tab, i) => (
          <div key={i} className={`${s.tab} ${i === currentTab ? s.active : ''}`}>
            <p>
