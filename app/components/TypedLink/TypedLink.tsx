...
export default function TypedLink({ hoverAnimate, viewAnimate = true, ...props }: Props) {
  ...
  const animateLink = contextSafe(() => {
    ...
    chars.forEach((char, i) => {
      ...
      gsap
        .timeline({
          defaults: { duration: 0.03, repeatDelay: 0.04 },
        })
        .fromTo(char, { opacity: 0 }, { opacity: 1, delay: (i + 1) * 0.07 })
        .to(
          char,
          {
            repeat: 4,
  ...
  return <Link {...props} className={s.link} ref={container} onMouseEnter={animateLink} />
}
