...
export class StrapiRequest {
    ...
    constructor(params: StrapiRequestParameters) {
        ...
        this.queryParameters = this.stringifyQueryParameters(
            this.attributes.parameters,
        );
    }

    private stringifyQueryParameters(
        params: StrapiRequestParameters["parameters"] = {},
    ): string {
        const query_params: string[][] = [];

        params.sort &&
            query_params.push(this.stringifySortParameters(params.sort));
        params.fields &&
            query_params.push(this.stringifyFieldsParameters(params.fields));
        params.filters &&
            query_params.push(this.stringifyFiltersParameters(params.filters));
        params.populate &&
            query_params.push(
                this.stringifyPopulateParameters(params.populate),
            );
    ...
    private stringifyFiltersParameters(filters: Filters): string[] {
        const stringify_key_params = (keys: Filter["key"]) =>
            Array.isArray(keys) ? stringify_array(keys) : `[${keys}]`;
        const stringify_array = (keys: string[]) =>
            keys.map((key) => `[${key}]`).join("");

        return filters.map(
            (filter) =>
                `filters${stringify_key_params(filter.key)}[${filter.operator}]=${filter.value}`,
        );
    }
